...
def importar_arquivos():
    ...
    try:
        ...
        for i, arquivo in enumerate(arquivos):
            ...
            progresso.set((i + 1) / total_arquivos)
            janela.update_idletasks()
...
frame_arquivos.grid(row=5, column=1, pady=5, sticky="nsew")
...
btn_importar.grid(row=6, column=1, pady=30, sticky="nsew")

# Caixa de log
caixa_log = scrolledtext.ScrolledText(janela, width=60, height=10, wrap=ctk.WORD)
caixa_log.grid(row=7, column=1, sticky="nsew")

# Barra de progresso
# progresso = ctk.CTkProgressBar(janela, orientation="horizontal", width=400)
# progresso.grid(row=7, column=1, columnspan=2, padx=10, pady=10)
# progresso.set(0)
